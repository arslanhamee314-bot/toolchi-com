...
import { BookOpen, Clock, Mail, ChevronRight } from "lucide-react";
...
export default function BlogContent() {
  ...
  const handleSubscribe = (e: React.FormEvent) => {
    ...
    try {
      const subs = JSON.parse(localStorage.getItem("toolchi_newsletter_subs") || "[]");
      localStorage.setItem("toolchi_newsletter_subs", JSON.stringify([subscription, ...subs]));
      console.log("Newsletter simulated subscription:", subscription);
    } catch (err) {}
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
      ...
      <div className="lg:col-span-8 flex flex-col gap-10">
        ...
        {selectedCategory === "all" && featuredPost && (
          <article className="bg-white dark:bg-card border border-border rounded-3xl overflow-hidden card-shadow flex flex-col md:flex-row group transition-all duration-300 hover:-translate-y-0.5">
            ...
            <div className="w-full md:w-1/2 p-6 md:p-8 flex flex-col justify-between gap-5 text-left">
              <div className="flex flex-col gap-3">
                <span className="text-3xs font-extrabold text-[#7d4dff] bg-[#f3eeff] dark:bg-[#1e1a35] px-2.5 py-1 rounded-md w-fit uppercase tracking-wider">
                  Featured Guide
                </span>
        ...
        {gridPosts.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {gridPosts.map((post) => (
              <article
                key={post.slug}
                className="bg-white dark:bg-card border border-border rounded-2xl overflow-hidden card-shadow flex flex-col justify-between min-h-96 group transition-all duration-300 hover:-translate-y-0.5"
              >
                ...
                <div className="p-5 flex-1 flex flex-col gap-3.5 justify-between text-left">
      ...
      <aside className="lg:col-span-4 flex flex-col gap-6 text-left">
        ...
        <div className="bg-[#f3eeff] dark:bg-[#151923] border border-[#e8ddff]/80 dark:border-border/60 rounded-3xl p-6 flex flex-col gap-4">
