...
import { Terminal, ArrowLeft, Key, Code, CheckCircle, Cpu } from "lucide-react";
import Header from "@/components/Header";

export default function AiBloggingApiDocs() {
      ...
      <main className="max-w-4xl mx-auto px-6 py-12 flex flex-col gap-10">
        ...
        <section className="flex flex-col gap-4">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-3xl font-black text-[#111827] dark:text-white">AI Content Generation API</h1>
          </div>
        ...
        <section className="flex flex-col gap-6">
          <h2 className="text-lg font-black border-b border-border/40 pb-2">POST /api/v1/ai/blog-pack</h2>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3 font-semibold">
                <div className="p-3 bg-neutral-50 dark:bg-card/30 rounded-xl border border-border/40 flex justify-between items-start gap-4">
                  <div>
                    <span className="font-mono text-primary">topic</span>
                    <span className="text-[10px] text-rose-500 block mt-0.5">Required &bull; string</span>
                  </div>
                  <p className="text-muted-foreground text-right max-w-xs">Base title focus or target topic query keywords.</p>
                </div>
                <div className="p-3 bg-neutral-50 dark:bg-card/30 rounded-xl border border-border/40 flex justify-between items-start gap-4">
                  <div>
                    <span className="font-mono text-primary">tone</span>
                    <span className="text-[10px] text-muted-foreground block mt-0.5">Optional &bull; string</span>
                  </div>
                  <p className="text-muted-foreground text-right max-w-xs">Style tone (professional, casual, bold). Default is professional.</p>
                </div>
                <div className="p-3 bg-neutral-50 dark:bg-card/30 rounded-xl border border-border/40 flex justify-between items-start gap-4">
                  <div>
                    <span className="font-mono text-primary">max_suggestions</span>
                    <span className="text-[10px] text-muted-foreground block mt-0.5">Optional &bull; integer</span>
                  </div>
                  <p className="text-muted-foreground text-right max-w-xs">Total title recommendations returned. Default is 5.</p>
                </div>
              </div>
            </div>
...
console.log("Suggested titles:", data.titles);`}
            </pre>
          </div>
        </section>
        ...
        <section className="p-6 bg-primary/5 rounded-3xl border border-primary/10 flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="space-y-1.5 text-center md:text-left">
            <h4 className="font-extrabold text-sm text-[#111827] dark:text-white">Ready to automate content outlining pipelines?</h4>
